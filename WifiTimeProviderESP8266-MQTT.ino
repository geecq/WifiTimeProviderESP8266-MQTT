...
#define SOFTWARE_VERSION "v357mjs.9"
#define SERIAL_NUMBER "0x0x0x"
...
/******  START MQTT code addition  ******/
bool Connect() {
  if (client.connect(clientID, mqtt_username, mqtt_password)) {
    client.subscribe("Command");                /** <---- List all subscriptions here **/
    return true;
  } else return false;
}
...
void ReceivedMessage(char* topic, byte* payload, unsigned int length) {
  ...
  if (msg.indexOf("Nixie Clock") != -1)  {      /** <---- "Nixie Clock" used as string identifier  **/
    ...
    if (msg.indexOf("Status") != -1) {          /** <---- 'Status' report requested, see readme file  **/
      switch (configDayBlanking) {
        case 0:
          ...
          break;
        case 1:
          clockIsOn = !(numDayOfWeek == 6 || numDayOfWeek == 7);
          displayMode = "1: Off Weekends";
          displayMode += "\nNixie clock is ";
          if (clockIsOn) displayMode += "On";
            else displayMode += "Off";
          break;
        case 2:
          clockIsOn = !(numDayOfWeek > 0 && numDayOfWeek < 6);
          displayMode = "2: Off Weekdays";
          displayMode += "\nNixie clock is ";
          if (clockIsOn) displayMode += "On";
            else displayMode += "Off";
          break;
        case 3:
          clockIsOn = false;
          displayMode = "3: Always Off";
          break;
        case 4:
          displayMode = "4: Selected Hours - On Time ";
          displayMode += String(configBlankTo).c_str();
          displayMode += ":00, Off Time ";
          displayMode += String(configBlankFrom).c_str();
          displayMode += ":00";
          if (configBlankFrom > configBlankTo)
            clockIsOn = (currHour >= configBlankTo && currHour < configBlankFrom);
          else 
            clockIsOn = (currHour >= configBlankTo && currHour > configBlankFrom);
          displayMode += "\nNixie clock is ";
          if (clockIsOn) displayMode += "On";
      ...
      if (clockIsOn) {
        msg += F("\nLED Colors:");
        msg += F("\n  Red Channel = ");
        msg += String(configRedCnl).c_str();
        msg += F("\n  Green Channel = ");
        msg += String(configGreenCnl).c_str();
        msg += F("\n  Blue Channel = ");
    ...
    if (msg.indexOf("Mode 0") != -1) {          /** <---- clock mode change commands begin here, see readme file **/
      byte newDayBlanking = 0;
      if (newDayBlanking != configDayBlanking) {
        setClockOptionDayBlanking(newDayBlanking);
        clockIsOn = true;
        mqtt_Publish("Information", "Clock Mode 0: Always On");
      }
    }

    if (msg.indexOf("Mode 1") != -1) {
      byte newDayBlanking = 1;
      if (newDayBlanking != configDayBlanking) {
        setClockOptionDayBlanking(newDayBlanking);
        clockIsOn = !(numDayOfWeek == 6 || numDayOfWeek == 7);
        mqtt_Publish("Information", "Clock Mode 1: Off Weekends");
      }
    }

    if (msg.indexOf("Mode 2") != -1) {
      byte newDayBlanking = 0;
      if (newDayBlanking != configDayBlanking) {
        setClockOptionDayBlanking(newDayBlanking);
        clockIsOn = !(numDayOfWeek > 0 && numDayOfWeek < 6);
        mqtt_Publish("Information", "Clock Mode 2: Off Weekdays");
      }
    }

    if (msg.indexOf("Mode 3") != -1) {
      byte newDayBlanking = 3;
      if (newDayBlanking != configDayBlanking) {
        setClockOptionDayBlanking(newDayBlanking);
        configDayBlanking = newDayBlanking;
        clockIsOn = false;
        mqtt_Publish("Information", "Clock Mode 3: Always Off");
      }
    }

    if (msg.indexOf("Mode 4 On=") != -1) {
      byte newBlankTo = (msg.substring(22)).toInt();
      if (newBlankTo < 0 || newBlankTo > 23 || newBlankTo == configBlankTo) return;
      byte newDayBlanking = 4;
      if (newDayBlanking != configDayBlanking) {
        setClockOptionDayBlanking(newDayBlanking);
        configDayBlanking = newDayBlanking;
      }
      setClockOptionBlankTo(newBlankTo);
      configBlankTo = newBlankTo;
      msg = F("Clock Mode 4: On Time ");
      msg += String(newBlankTo).c_str();
      msg += F(":00");
      mqtt_Publish("Information", msg.c_str());
      msg = "";
    }

    if (msg.indexOf("Mode 4 Off=") != -1) {
      byte newBlankFrom = (msg.substring(23)).toInt();
      if (newBlankFrom < 0 || newBlankFrom > 23 || newBlankFrom == configBlankFrom) return;
      byte newDayBlanking = 4;
      if (newDayBlanking != configDayBlanking) {
        setClockOptionDayBlanking(newDayBlanking);
        configDayBlanking = newDayBlanking;
      ...
      configBlankFrom = newBlankFrom;
      msg = F("Clock Mode 4: Off Time ");
      msg += String(newBlankFrom).c_str();
      msg += F(":00");
      mqtt_Publish("Information", msg.c_str());
      msg = "";
    }

    if (msg.indexOf("Mode 4") != -1) {
      byte newDayBlanking = 4;
      if (newDayBlanking != configDayBlanking) {
        setClockOptionDayBlanking(newDayBlanking);
        configDayBlanking = newDayBlanking;
      ...
      mqtt_Publish("Information", "Clock Mode 4: On Time 8:00, Off Time 24:00");
    }

    if (msg.indexOf("Red=") != -1) {            /**  RGB settings commands begin here **/
      byte newRedCnl = (msg.substring(16)).toInt();
      if (newRedCnl < 0 || newRedCnl > 15) newRedCnl = configRedCnl;
      if (newRedCnl != configRedCnl) {
        setClockOptionRedCnl(newRedCnl);
        configRedCnl = newRedCnl;
      }
      msg = F("Red channel = ");
      msg += String(newRedCnl).c_str();
      mqtt_Publish("Information", msg.c_str());
    }

    if (msg.indexOf("Green=") != -1) {
      byte newGreenCnl = (msg.substring(18)).toInt();
      if (newGreenCnl < 0 || newGreenCnl > 15) newGreenCnl = configGreenCnl;
      if (newGreenCnl != configGreenCnl) {
        setClockOptionGrnCnl(newGreenCnl);
        configGreenCnl = newGreenCnl;
      }
      msg = F("Green channel = ");
      msg += String(newGreenCnl).c_str();
      mqtt_Publish("Information", msg.c_str());
    }

    if (msg.indexOf("Blue=") != -1) {
      byte newBlueCnl = (msg.substring(17)).toInt();
      if (newBlueCnl < 0 || newBlueCnl > 15) newBlueCnl = configBlueCnl;
      if (newBlueCnl != configBlueCnl) {
        setClockOptionBluCnl(newBlueCnl);
        configBlueCnl = newBlueCnl;
      }
      msg = F("Blue channel = ");
      msg += String(newBlueCnl).c_str();
      mqtt_Publish("Information", msg.c_str());
    }
  }
}

bool mqtt_Publish(const char* mqtt_Topic, const char* msg) {
...
/******  START MQTT main loop code addition  ******/
  if (!client.connected()) {
    Connect();
  }
  client.loop();
/******  END MQTT main loop code addition  ******/
...
boolean getClockOptionsFromI2C() {
  ...
  // now get the data
  byte dataLen = 0;
  if ( i2cProtocol == v1v54 ) {
    dataLen = I2C_DATA_SIZE_V1;
  }
  if ( i2cProtocol == v2v62 ) {
    dataLen = I2C_DATA_SIZE_V2;
  }
